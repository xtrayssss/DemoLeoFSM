...
namespace Leopotam.EcsLite.Packages.ECS.src
{
    ...
    public class EcsSystems : IEcsSystems
    {
        ...
        public virtual void Init()
        {
                ...
                if (system is IEcsPreInitSystem initSystem)
                {
                    ...
                    if (worldName != null)
                    {
                        throw new System.Exception(
                            $"Empty entity detected in world \"{worldName}\" after {initSystem.GetType().Name}.PreInit().");
                    }
#endif
                }
            }
            ...
            foreach (var system in _allSystems)
            {
                if (system is IEcsInitSystem initSystem)
                {
                    ...
                    {
                        throw new System.Exception(
                            $"Empty entity detected in world \"{worldName}\" after {initSystem.GetType().Name}.Init().");
                    }
#endif
                }
            }
        ...
        public virtual void Run()
        {
            ...
            for (int i = 0, iMax = _runSystems.Count; i < iMax; i++)
            {
                ...
                {
                    throw new System.Exception(
                        $"Empty entity detected in world \"{worldName}\" after {_runSystems[i].GetType().Name}.Run().");
                }
#endif
            }
            ...
            for (int i = 0, iMax = _postRunSystems.Count; i < iMax; i++)
            {
                ...
                {
                    throw new System.Exception(
                        $"Empty entity detected in world \"{worldName}\" after {_postRunSystems[i].GetType().Name}.PostRun().");
                }
#endif
            }
        }
        ...
        public virtual void Destroy()
        {
            for (var i = _allSystems.Count - 1; i >= 0; i--)
            {
                if (_allSystems[i] is IEcsDestroySystem destroySystem)
                {
                    ...
                    {
                        throw new System.Exception(
                            $"Empty entity detected in world \"{worldName}\" after {destroySystem.GetType().Name}.Destroy().");
                    }
#endif
                }
            }
            ...
            for (var i = _allSystems.Count - 1; i >= 0; i--)
            {
                if (_allSystems[i] is IEcsPostDestroySystem postDestroySystem)
                {
                    ...
                    {
                        throw new System.Exception(
                            $"Empty entity detected in world \"{worldName}\" after {postDestroySystem.GetType().Name}.PostDestroy().");
                    }
#endif
                }
            }
